== Attacks

=== Barrier Programs

Barrier programs are short programs used to measure the capabilities of Cortex.
This chapter contains the barrier programs which Cortex should be able to solve, or are on the roadmap to be solved.

include::{snippets}/BarrierProgram.adoc[]

=== Attack Generators

Below is a demonstration of Cortex analysing and finding the solution to barrier programs, available also is the
time it took for Cortex to construct the solution.

Cortex will try to reach `HALT WINNER`, or `CALL` with a recipient address of `1234`.

include::{snippets}/Attacker.adoc[]

include::{snippets}/AttackProgramBuilder.adoc[]

=== Exploration

Below is a demonstration of Cortex navigating through every possible path of the barrier programs using purely symbolic execution.

Documented are also the solution to the first path encountered which ended in `HALT WINNER`.

include::{snippets}/SymbolicProgramExplorer.adoc[]
