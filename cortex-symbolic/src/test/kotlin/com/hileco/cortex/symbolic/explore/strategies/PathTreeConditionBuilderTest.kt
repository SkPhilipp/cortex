...
class PathTreeConditionBuilderTest {
    ...
    private fun testExpression(constant: Long): Expression {
        return Expression.Equals(Expression.Reference(ProgramStoreZone.CALL_DATA, Expression.Value(constant)), Expression.Value(constant))
    }

    @Test
    fun testRootWithBranches() {
        val stack = LayeredVmStack<SymbolicPathEntry>()
        stack.push(SymbolicPathEntry(0, Expression.Value(0), true, testExpression(0)))
        val branch = stack.copy()
        stack.push(SymbolicPathEntry(0, Expression.Value(0), true, testExpression(1)))
        branch.push(SymbolicPathEntry(0, Expression.Value(0), true, testExpression(2)))
